...
signed-in user. Choosing a language saves the preference in the `ezrepo-locale` browser cookie and applies it on
future visits.

English is the deterministic default and fallback. When a browser preference or saved cookie does not identify a
supported locale, ezRepo renders English. Date, time, and number formatting use the selected locale.
...
5. Run `pnpm --filter @ezrepo/web test`, `pnpm --filter @ezrepo/web typecheck`, and the affected Playwright test.

Provider names, repository names, workflow names, and other provider-supplied data remain unchanged because they are
not ezRepo interface copy.
